...
if ($aksi=="Tampilkan") {
 			...
 			$href="index.php?pilihan=alihat&aksi=Tampilkan";
		...
		if (trim($id)!="") {
			...
			if (trim($kunci)!="") {
				$qfield.=" AND (SAMPEL LIKE '%$kunci%' OR CATATANS LIKE '%$kunci%') ";
		...
		if ($sort=="") {
		 ...
		 $q="SELECT COUNT(permintaan.ID) AS JML  FROM permintaan,minta 
		WHERE 
		...
		*,
		...
		if (sqlnumrows($h)>0) {
			...
			echo "
				...
				<table $borderdata class=data>
					<tr class=judulkolom align=center valign=middle>
						...
						<td nowrap width='10%'><a href='$href&sort=ID2'>Kode</td>
 						<td nowrap ><a href='$href&sort=TANGGALDATANG'>Tgl Masuk</td>
						<td nowrap ><a href='$href&sort=TANGGALDEADLINE'>Tgl Deadline</td>
						<td nowrap ><a href='$href&sort=permintaan.TANGGALSELESAI'>Tgl Selesai</td>
			...
			while($data=sqlfetcharray($h)) {
				$kelas=kelas($i);
        $labelaksi="Lihat";
				if ($data[STATUSS]==2 || $data[STATUSSS]==3 || $data[STATUSSS]==6) {
          $labelaksi="Update";
        }
				echo "
					<tr valign=top $kelas>
						...
						<td nowrap align=center><a href='index.php?pilihan=aupdate&idupdate=$data[IDTRANS]&idsupdate=$data[ID2]&kurva=$kurva'>$labelaksi</td>
 					</tr>";
				$i++;
			}
...
<form name=form action=index.php method=post>
	...
	<tr>
		...
		<tr>
			<td width=20%  nowrap>
				Kata Kunci Sampel/Catatan Supervisor
			</td>
			<td>
