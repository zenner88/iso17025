...
if ($aksi=="Tampilkan") {
 			...
 			$href="index.php?pilihan=slihat&aksi=Tampilkan";
		...
		if (trim($id)!="") {
			...
			if (trim($kunci)!="") {
				$qfield.=" AND (SAMPEL LIKE '%$kunci%' OR CATATANM LIKE '%$kunci%') ";
		...
		*,
		...
		if (sqlnumrows($h)>0) {
				...
				<table $borderdata class=data>
					<tr class=judulkolom align=center valign=middle>
						...
						<td nowrap ><a href='$href&sort=permintaan.STATUS'>Status</td>
  						<td nowrap ><a href='$href&sort=IDAN'>Analis</td>
						<td nowrap >Aksi</td>
					</tr>";
			...
			while($data=sqlfetcharray($h)) {
				$kelas=kelas($i);
        $labelaksi="Lihat";
				if ($data[STATUSS]==1 || $data[STATUSSS]==4) {
          $labelaksi="Update";
        }
				echo "
					<tr valign=top $kelas>
						...
						<td nowrap align=center><a href='index.php?pilihan=supdate&idupdate=$data[IDTRANS]&idsupdate=$data[ID2]'>$labelaksi</td>
 					</tr>";
				$i++;
			}
...
<form name=form action=index.php method=post>
	...
	<tr>
		...
		<tr>
			<td width=20%  nowrap>
				Kata Kunci Sampel/Catatan Manager Teknis
			</td>
			<td>
