...
if ($aksi=="Tampilkan") {
        ...
        foreach ($daftarnilai as $k=>$v) {
          $q="INSERT INTO nilaibaku (ID,NILAI) VALUES ('$k','$v') ";
          doquery($q,$koneksi);
          if (sqlaffectedrows($koneksi)<=0) {
            $q="UPDATE nilaibaku SET NILAI='$v' WHERE ID='$k'";
            doquery($q,$koneksi);
            if (sqlaffectedrows($koneksi)>0) {
        		  $ketlog="Simpan Nilai Baku Jenis Uji. ID=$k, Nilai=$v ";
              buatlogiso(25,$ketlog,$q,$users);
              $berhasil++;
            }
          } else {
      		  $ketlog="Simpan Nilai Baku Jenis Uji. ID=$k, Nilai=$v ";
        ...
        $errmesg="$berhasil data nilai baku default telah disimpan.";
...
/*		$q="SELECT * FROM jenisuji 
		...
		*/
		...
		if (is_array($arrayanalisis)) {
			...
			echo "<center><h3>Update Nilai Baku Parameter Jenis Uji</h3>";
			printmesg($errmesg);
      ...
      <form name=form action=index.php method=post>
      	...
      	<input type=hidden name=aksi value='$aksi'>
				...
				<table $borderdata class=data>
					<tr class=judulkolom align=right valign=middle >
  						<td nowrap colspan=6><input type=submit name=aksitambahan value='Update'></td>
					</tr>					
          <tr class=judulkolom align=center valign=middle>
						...
						<td nowrap ><a href='$href&sort=RUMUS'>Satuan</td>
 						<td nowrap >Nilai Baku Default</td>
					</tr>";
  				...
  				echo "
  					<tr valign=top $kelas>
  						...
  						<td align=center ><b>".$arrayanalisissatuan[$k]."</td>
   						<td align=center ><input type=text size=15 name='daftarnilai[$k]' value='".getnilaibaku($k)."'></td>
   					</tr>";
  				$i++;
				}
			}
...
if ($aksi=="") {
echo "
<h3>Nilai Baku Parameter Jenis Uji</h3>";
printmesg($errmesg);
echo "
<form name=form action=index.php method=post>
	<input type=hidden name=pilihan value='$pilihan'>
