...
if ($aksi=="Tampilkan") {
		...
		if ($aksitambahan=="hapus") {
 				...
 				$q="DELETE FROM minta WHERE ID='$idhapus' AND STATUS=4 AND IDKLIEN='$users'";
				doquery($q,$koneksi);
				if (sqlaffectedrows($koneksi)>0) {
    		  $ketlog="Hapus permintaan. ID=$idhapus, Klien=$users";
		...
		if (trim($id)!="") {
			...
			$href.="&id=$id";
 				...
 				$qfield.=" AND minta.IDKLIEN='$users' ";
				$judul.=" Klien = ".getnamatoko($users)." <br>";
				$href.="&idklien=$users";
 			if ($statuspermintaan!="") {
		  	$qfield.=" AND minta.STATUS='$statuspermintaan' ";
		...
		if ($sort=="") {
		 ...
		 FROM minta  LEFT JOIN survey ON minta.ID=survey.ID
		WHERE minta.IDKLIEN='$users'
		$qfield
...
WHERE minta.STATUS='1' AND minta.IDKLIEN='$users' AND survey.ID IS NULL "; // SELESAI
$h=doquery($q,$koneksi);
echo mysql_error();
$d=sqlfetcharray($h);
$errmesg2="Ada $d[JML] permintaan analisis yang harus diperiksa. Silakan <a href='index.php?pilihan=$pilihan&statuspermintaan=1&survey=1&aksi=Tampilkan'>klik di sini </a>untuk melihat data tersebut.";
printmesg($errmesg2);
